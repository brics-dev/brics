...
public class BaseAction extends ActionSupport {
	...
	// These should not be injected here and should be moved to a higher level "base" action
	// Because there is no guarantee that the module will have the database tables for this manager
	...
	/*
	 * Returns the number of withdrawn requests for the admin dashboard
	 */
	...
	public String getLastDeployed() throws IOException{
		return modulesConstants.getLastDeployedTimeStamp();
	}
	...
	public String checkUrlPrefix(String url) {
		...
		if (url != null
				&& !(url.toLowerCase().startsWith(ServiceConstants.URL_PREFIX_HTTP)
						|| url.toLowerCase().startsWith(ServiceConstants.URL_PREFIX_HTTPS) || url.toLowerCase()
						.startsWith(ServiceConstants.URL_PREFIX_FTP))) {
	...
	protected List<EntityMap> getEntitiesForCurrentUser(PermissionType permission, EntityType entity, boolean getPublic)
		...
		//logger.debug("Account: " + getAccount().getUserName() + " Permission: " + permission.getName());

		List<EntityMap> nonWSList = restProvider.listUserAccessEntities(entity, permission, getPublic);
		//logger.debug("This is the number of results back " + nonWSList.size());
		return nonWSList;
	}
