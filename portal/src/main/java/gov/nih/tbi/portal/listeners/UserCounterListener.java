...
public class UserCounterListener implements ServletContextListener, HttpSessionAttributeListener
{
    ...
    private int counter;
    private Set<AccountUserDetails> users;
    ...
    synchronized void addUsername(AccountUserDetails user)
    {

        users = (Set<AccountUserDetails>) servletContext.getAttribute(USERS_KEY);

        if (users == null)
        {
            users = new LinkedHashSet<AccountUserDetails>();
        }
    ...
    @SuppressWarnings("unchecked")
    synchronized void removeUsername(AccountUserDetails user)
    {
    ...
    public void attributeAdded(HttpSessionBindingEvent event)
    {
        ...
        if (event.getName().equals(EVENT_KEY) && !isAnonymous())
        {
            ...
            if (securityContext.getAuthentication().getPrincipal() instanceof AccountUserDetails)
            {
                AccountUserDetails user = (AccountUserDetails) securityContext.getAuthentication().getPrincipal();
                addUsername(user);
            }
        }
    }
    ...
    /**
     * Needed for Acegi Security 1.0, as it adds an anonymous user to the session and then replaces it after
     * authentication. http://forum.springframework.org/showthread.php?p=63593
     * 
     * @param event
     *            the session binding event
    ...
    public void attributeReplaced(HttpSessionBindingEvent event)
    {
        ...
        if (event.getName().equals(EVENT_KEY) && !isAnonymous())
        {
            ...
            if (securityContext.getAuthentication() != null
                    && securityContext.getAuthentication().getPrincipal() instanceof AccountUserDetails)
            {
                final AccountUserDetails user = (AccountUserDetails) securityContext.getAuthentication().getPrincipal();
                addUsername(user);
            }
        }
    }
