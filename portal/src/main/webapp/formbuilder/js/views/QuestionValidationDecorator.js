...
var QuestionValidationDecorator = {
	                	...
	                	match : function(model){
	                		...
	                		if(answerType==String(Config.answerType.numeric)){
	                			if(rangeOper==Config.rangeOperator.between){
	                				...
	                				//only do following 2 maybe if q  type is visual scale:
	...
	renderValidationCommons : function(model) {
		...
		if (type === Config.questionTypes.textbox) {
			...
			if(FormBuilder.form.get("isCAT") && FormBuilder.form.get("measurementType") != 'shortForm'){
				contentHtml = TemplateManager.getTemplate("editQuestionValidationTabForCAT");
			}else{
				contentHtml = TemplateManager.getTemplate("editQuestionValidationTab");				
			}	
	...
	renderFieldRequired : function() {
		if (this.model.get("required")) {
			...
			this.$("#required").html('<input type="checkbox" name="required" value="true" />');
		}
	},
	...
	renderFieldRangeOperator : function() {
		...
		switch(rangeOperatorValue) {
			...
			case "2": // fall through
			case 2:
				outputValue = "Less Than or Equal To" + val1;
				break;
			case "3": // fall through
			case 3:
				outputValue = "Greater Than or Equal To" + val1;
				break;
			case "4": // fall through
			case 4:
