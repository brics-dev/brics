		...
		validationRules : [
		                   ...
		                   new ValidationRule({
		                	   ...
		                	   match	: function(model) {
		                		   ...
		                		   var calculation = model.get("calculation");
		                		   var sId = model.get("sectionId");
		                		   var qId = "_Q_"+model.get("questionId");
		                		   var qstnToEdit = sId + qId;
		                		   
		                		   
		                		   var calcArray = this.getCalcRuleInArray(calculation);
		                		   var allQtnsCalcRule = {};
		                		   ...
		                		   FormBuilder.form.getSectionsInPageOrder().forEach(function(section) {
		                			   ...
		                			   sectionQuestions.forEach(function(question) {
		                				   var calc = question.get("calculation");
		                				   if(calc != null && calc.trim()!='') {
		                					   allQtnsCalcRule[section.get("id")+"_Q_"+question.get("questionId")] = calc;
		                				   }		                				   
		                			   });
		                			   
		                		   });
		                		   
		                		   //check if the question selected to edit is part of other question's 
		                		   //calculation rule and causing circular logic
		                		   //Ex1: if A=B+C, C=A+D, then error message will be displayed 
		                		   //Ex2: if A=B+C, B=D+E, E=A+F then error message will be displayed
		                		   for(var i=0; i<calcArray.length; i++){
		                			   if(calcArray[i] in allQtnsCalcRule){
		                				   var calculationRule = allQtnsCalcRule[calcArray[i]];
		                				   if(calculationRule.indexOf(qstnToEdit) > -1){
		                					   dename = this.getDataElementName(calcArray[i]);
		                					   this.description = Config.language.circularDependency + dename +". "+ Config.language.correctbeforesave;
		                					   return false;
		                				   }else{
		                					   var crArray = this.getCalcRuleInArray(calculationRule);
		                					   if(crArray.length > 0){
		                						   for(var j=0; j<crArray.length; j++){
		                							   if(calcArray.indexOf(crArray[j]) == -1){
		                								   calcArray.push(crArray[j]);
		                							   }
			                						}
			                					}
			                				}
		                	   ...
		                	   getCalcRuleInArray : function(calcRule){
		                		   var calcRuleArray = new Array();
		                		   var findAllQstns = /\[S_[-]?\d+_Q_([0-9]+)\]/g;
		                		   
		                		   //Get questions used in the calculation rule and put it in array
		                		   if(!!calcRule){
			                		   while(result = findAllQstns.exec(calcRule)){
			                			   result[0] = result[0].replace("[","");
			                			   result[0] = result[0].replace("]","");
			                			   if(calcRuleArray.indexOf(result[0]) == -1){
			                				   calcRuleArray.push(result[0]);
			                			   }
			                		   }
		                	   		}
		                		   return calcRuleArray;
		                	   },
		...
		renderCalculationRuleCommons: function(model) {
			...
			if(type === Config.questionTypes.textbox) {
				...
				if(FormBuilder.form.get("isCAT") && FormBuilder.form.get("measurementType") != 'shortForm'){
					contentHtml = TemplateManager.getTemplate("editQuestionCalculationRuleTabForCAT");
				}else{
					contentHtml = TemplateManager.getTemplate("editQuestionCalculationRuleTab");
				}
