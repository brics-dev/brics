var Form = BaseModel.extend({ 
	defaults : {
		...
		status : "Draft",
		submitData : {
			existingsectionIdsToDeleteArray : [],  //are these even being used???
			existingQuestionIdsToDeleteArray : []  //are these even being used???
		},
		/* added by Ching Heng */
		formtypeid : 10,
	...
	allGroups : null,
	existingQuestionsToDelete : new Array(),
	...
	addSection : function(config, calcId) {
		if (typeof calcId === "undefined" || calcId) {
			...
			var newId = this.sections.getNextSectionId();
			config.id = newId;
	...
	*/
	...
	/**
	 * Method to get Question by QuestionId and sectionId 
	 */
	getQuestionByQuestionIdSectionId : function(questionId,sectionId) {
		var output = null;
		FormBuilder.form.sections.forEach(function(section) {
			var secId = Number(section.get("id").substr(section.get("id").indexOf("S_") + 2));
			if(Number(sectionId)==secId){
				var question = section.getQuestionById(Number(questionId));
				if (typeof question !== "undefined") {
					output = question;
					return;
				}
	...
	/**
	 * Method to get Section by  sectionId
	 */
	getSectionBySectionId : function(sectionId) {
		var output = null;
		FormBuilder.form.sections.forEach(function(section) {
			var secId = Number(section.get("id").substr(section.get("id").indexOf("S_") + 2));
			if(Number(sectionId)==secId){
	...
	findChildQuestions : function(properties) {
		var output = [];
		this.sections.forEach(function(sect) {
			if(sect.repeatedSectionParent==properties.sectionId){
				var question = sect.questions.findWhere({questionId : properties.questionId});
				if (typeof question !== "undefined") {
					output.push(question);
				}
			}
	...
	loadDataElements : function(dataElementsJSON) {
		...
		for (var t=0; t<dataElementsJSON.length; t++) {
			var de = {
				...
				catOid				:	dataElementsJSON[t].catOid,
				formItemOid			:	dataElementsJSON[t].formItemOid
			};
	...
	mappingRules : function(params,KEYs,key){
		...
		case 'tabdisplay':
			...
			break;
		case 'name':
			return 'formForm.'+KEYs[key]+'='+encodeURIComponent(params[KEYs[key]])+'&';
			break;
		case 'access':
