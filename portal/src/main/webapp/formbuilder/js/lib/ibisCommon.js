...
/**
 * Make the labels look a little nicer
 * 
 * This takes all labels inside formrows on the page and standardizes them
 * to be all the same width.  It adjusts for required inputs and hidden
 * elements.
 * 
 * It will look at the label to see its initial width.  If the width is zero
 * or "auto", it will try to calculate the width based on the font size and
 * number of characters in the label.
 */
var FormRowCalculator = {
	/**
	 * Initializes the formrow calculator.  Processes the elements on the page
	 * to size things correctly.  Also adds required field stars.
	 */
	init : function() {
	...
	sizeInputs : function() {
		...
		var rowWidth = $(".formrow_1:visible").eq(0).width();
		$(".formrow_1").each(function() {
	...
	/**
	 * Given a container formrow, set the width of the input half of the row
	 * to "the rest of the row width".
	 * 
	 * @param $row the row that contains the input to resize
	 * @param containerSize the size of the container (max total width)
	 */
	setInputSize : function($row, containerSize) {
		...
		if ($input.attr("type") != "radio" 
			...
			$input.width(containerSize - labelWidth - adjustWidth);
		}
	},
...
var Utils = {
	 ...
	 * 
	 * @param obj - The Javascript object that will be tested for existance.
...
//	$('input[type="button"].singleClick, button.singleClick').live("click", function() {
...
var HtmlInputHandler = {
	...
	/**
	 * Sets the Data information for each input to contain its original
	 * value.  This registers the input so that it can be reset back to
	 * the original value.
	 * 
	 * @param $form limits the set of inputs to a particular form.  Leave blank to do all
	 */
...
$(document).ready(function() {
	 ...
	 */
	$(".searchContainer form, form .searchContainer").append('<input type="hidden" name="searchSubmitted" value="" />');

	/**
	 * open advanced search divs if they have search data
	 * @return true if search container should stay open, otherwise false
	 */
	...
	/**
	 * original reference is to the p or div
	 * 
	 * makes h3+p, h3+div expandable/collapsible
	 */
...
/**
 * Resizes the main container to be the width of the window - 20 pixels for padding
 */
function resizeMainContainer() {
	...
	var newHeight = $(window).height() - $("#header").height() - $("#footerDiv").height() - parseInt($main.css('padding-bottom'));
	
	$("#main, #footer .content").width(newWidth);
	$("#navigation, #header .content").width(newWidth + 2); // modified for the borders
...
/**
 * fires after the window resizes.  This is a general hook and can be used for anything.
 * It waits for the resize to complete (waits 100ms) before actually firing the event
 * so that it does not fire windowResize() every millisecond.
 */
var resizeTimer;
