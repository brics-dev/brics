...
//			}
	...
	//		NOTE: the above will give an error if the data is not filtered
			var html = '<nav class="idtNav"><ul><li class="selectAllIcon"><img src="/portal/formbuilder/images/checkbox_unchecked.png" />';
			html += '<ul><li><a href="javascript:;" class="selectAllMenu selectAll"><img src="/portal/formbuilder/images/checkbox_checked.png" />Select All</a></li>';
			html += '<li><a href="javascript:;" class="selectAllMenu selectFilter"><img src="/portal/formbuilder/images/checkbox_indeterminate.png" />Select Filtered</a></li>';
			html += '<li><a href="javascript:;" class="selectAllMenu selectNone"><img src="/portal/formbuilder/images/checkbox_unchecked.png" />Select None</a></li>';
			html += '</ul></li></nav>';
	...
	selectAllIconToChecked : function() {
		var inputType = this.model.get("inputType");
		if (inputType != "none" && inputType != "radio") {
			this.$(".selectAllIcon > img").attr("src", "/portal/formbuilder/images/checkbox_unchecked.png");
		}
	},
	...
	onAddRow : function(model, collection) {
		// test: compare the length of rows in the datatable's storage vs the length of this.model.rows
		// if they are the same, don't add the row to the datatable
		// reasoning: 
		// 		case: two lists both contain the same list but the models are lagging behind
		//		action: don't add to the datatable and don't redraw
		// 		
		//		case: actually adding a new row.  rows has more entries than the datatable because the new model was already added
		//		action: add to the datatable and redraw
		if (this.model.get("rendered") === true) {
			var tableRowsCount = this.model.get("datatable").fnGetData().length;
			if (this.model.rows.length > tableRowsCount) {
				var dt = this.model.get("datatable");
				dt.fnAddData(model.get("data"), true);
			}
		}
	},
	...
	selectFiltered : function(event) {
		...
		if ( !this.model.get("disableSelectAll") ) {
			...
			try {
				var rowData = datatable.api(!0).rows({
					search: 'applied'
				});
				var rowCount = rowData[0].length;
				var saveValsToSelect = [];
				for (var i = 0; i < rowCount; i++) {
					var row = rowData.data()[i];
					var $input = $(row[0]);
					if ($input.length > 0) {
