...
function createStandardObjects()
{
	clientInfoObj = new clientInfo();	// Create browser info object
	clientInfoObj.init();	
	if(!dhtmlSuiteConfigObj){	// If this object isn't allready created, create it.
		dhtmlSuiteConfigObj = new config();	// Create configuration object.
		dhtmlSuiteConfigObj.init();
	}
	dhtmlSuiteCommonObj = new common();	// Create configuration object.
...
common.prototype = {
	...
	// {{{ init()
    /**
     * This method initializes the common object.
     *
     * @public
     */
...
dragDrop.prototype = {
	...
	init : function()
	{
		if(!standardObjectsCreated)createStandardObjects();	// This line starts all the init methods
	...
	{
        var bkGrd = document.getElementById('editContainerBk');
	...
	//	}
		...
		bkGrd.style.left = referenceToDragDropObject.dragObjCloneArray[referenceToDragDropObject.numericIdToBeDragged].style.left.replace('px','')/1;
        bkGrd.style.top = referenceToDragDropObject.dragObjCloneArray[referenceToDragDropObject.numericIdToBeDragged].style.top.replace('px','')/1; 
