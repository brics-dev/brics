...
cellColor        = "#CCCC99";     // TABLE CELL BG COLOR OF THE DATE CELLS IN THE BOTTOM FRAME
...
function showCalendar(dateField) {
    ...
    top.newWin = openPopup("javascript:parent.opener.calDocFrameset", "calWin", winPrefs);
...
function buildTopCalFrame() {
    ...
    var calDoc =
        ...
        "<CENTER>" +
        getMonthSelect() +
        "<INPUT NAME='year' id='year' VALUE='" + calDate.getFullYear() + "'TYPE=TEXT SIZE=5 MAXLENGTH=5 onChange='parent.opener.setYear()'>" +
        "</CENTER>" +
        "</TD>" +
        "</TR>" +
        "<TR>" +
...
function writeCalendar() {
    ...
    top.newWin.document.getElementById('bottomCalFrame').innerHTML = calDocBottom;
}
...
function isFourDigitYear(year) {
    ...
    if (year.length != 4) {
        YEAR.value = calDate.getFullYear();
        YEAR.select();
        YEAR.focus();
    }
    else {
...
function getMonthSelect() {
    ...
    monthSelect = "<SELECT NAME='month' id='month' onChange='parent.opener.setCurrentMonth()'>";
...
function returnDate(inDay)
{
   ...
   /*
    ...
    {
    	alert("Please select either today's date or a date in the past");
    	top.newWin.focus();
    	return;
	}
    */
