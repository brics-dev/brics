...
var OpButtonController = {
	...
	initTable : function($table) {
		 ...
		 */
		$('.dataTableContainer input[type="submit"], .dataTableContainer input[type="button"], .dataTableContainer button').prop('disabled', true);
		$('.dataTableContainer input[type="button"].alwaysEnabled, .dataTableContainer button.alwaysEnabled, .dataTableContainer input[type="button"].enabledOnNone, .dataTableContainer button.enabledOnNone').prop('disabled', false);
		$('.dataTableContainer input[type="submit"].alwaysEnabled, .dataTableContainer submit.alwaysEnabled, .dataTableContainer input[type="submit"].enabledOnNone, .dataTableContainer submit.enabledOnNone').prop('disabled', false);
	 ...
	 */
	selectedChange : function($checkbox) {
		var numSelected = IbisDataTables.countSelectedOptions(IbisDataTables.getTable($checkbox));
		IbisDataTables.getTableContainer($checkbox).find('input[type="submit"], input[type="button"], button').each(function(index, value){
			if (numSelected < 1) {
