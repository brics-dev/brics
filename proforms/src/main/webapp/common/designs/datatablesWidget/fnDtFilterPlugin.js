...
(function($, window, document, undefined) {
    ...
    <param = aName > =====> refers to the name of the select

    <param = iColumn > ========> refers to column index. aData[iColumn]

    <param = defaultVal > =======> refers to default filter value aocolumn.defaultVal
    <param = defaultVal > =======> refers to default filter value aoColumn.options[j].de
        **/
        var filterData = {};
        var array = [];

        function fnCreateSelectOptions(oTable, options, aName, iColumnIndex, bRegex, containerId, defaultVal, callBack) {
            if (!defaultVal){
                defaultVal = "";
            }
            var currentFilter = defaultVal;

            var r = '<select class="search_init select_filter form-control" id="' + aName + '"><option value="" name= ' + aName + ' class="search_init">' + aName + '</option>';
            var j = 0;
            var iLength = options.length;
            for (j = 0; j < iLength; j++) {
                if (typeof(options[j]) != 'object') {
                    var selected = '';
                    if (unescape(options[j]) == currentFilter ||
                    		unescape(options[j]) == unescape(currentFilter)
                    )
                    ...
                    if (bRegex) {
                        ...
                        r += '<option ' + selected + ' name= ' + aName + ' value="' + options[j].value + '">' + options[j].label + '</option>';
                    } else {
                        if (unescape(options[j].value) == currentFilter) selected = 'selected = selected';
                        r += '<option ' + selected + ' name= ' + aName + ' value="' + unescape(options[j].value) + '">' + options[j].label + '</option>';
                    }
                }
            }
            var select = $(r + '</select>');
        ...
        function filterSelectOptions(select, callBack, aName, currentFilter, iColumnIndex, bRegex) {
            ...
            // on change of value check if test method is defined if not run the default filter
            select.change(function() {
                ...
                if (typeof callBack == 'function' && callBack != 'undefined') {
                    ...
                    if (bRegex)
                        ...
                        oTable.fnFilter(unescape($(this).val()), iColumnIndex);
                    }
                }
            });
            ...
            for (j = 0; j < iLength; j++) {
                if (typeof(options[j]) == 'object') {
                    ...
                    if (bRegex) {
                        ...
                        r += '<li><input type=radio ' + checked + '  name="' + aName + '"  value="' + options[j].value + '">' + options[j].label + '</li>';
                    } else {
                        if (escape(options[j].value) == currentFilter) checked = 'checked = checked';
                        r += '<li><input type=radio ' + checked + '  name="' + aName + '"  value="' + escape(options[j].value) + '">' + options[j].label + '</li>';
                    }
                }
            }
        ...
        function createCheckBox(oTable, value, label, aName, iColumnIndex, bRegex, containerId, defaultValue, callBack) {
            ...
            if (bRegex) {
                ...
                r += '<input type=checkbox ' + checked + '  name="' + aName + '"  value="' + escape(value) + '">' + label;
            }
        ...
        /**
            aocolumn : {
                ...
                containerId: '#',
                name: '',
                bRegex: true or false,
                defaultValue: '',
                options: [{value: '', label: ''}] or ['test1', 'test2']
            }
        ...
        **/
        ...
        if ((properties.aoColumns !== null || properties.aoColumns !== 'undefined') && Array.isArray(properties.aoColumns)) {
            ...
            return (properties.aoColumns).forEach(function(aocolumn) {
                ...
                switch (aocolumn.type) {
                    ...
                    case 'select':
                        ...
                        fnCreateSelectOptions(oTable, aocolumn.options, aocolumn.name, aocolumn.columnIndex, aocolumn.bRegex, aocolumn.containerId, aocolumn.defaultValue, aocolumn.test);
                        break;

                    case 'radio':
                        createRadioButton(oTable, aocolumn.options, aocolumn.name, aocolumn.columnIndex, aocolumn.bRegex, aocolumn.containerId, aocolumn.defaultValue, aocolumn.test);
                        break;

                    case 'checkbox':
                        createCheckBox(oTable, aocolumn.value, aocolumn.label, aocolumn.name, aocolumn.columnIndex, aocolumn.bRegex, aocolumn.containerId, aocolumn.defaultValue, aocolumn.test);
                        break;
