...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
    	<input type="hidden" id="wr" value="{$webroot}"/>
    	<style type="text/css">
		...
		<table width="100%" style="border: {htmlAttributes/formBorder}px ridge; font-size:{/form/htmlAttributes/formFontSize}pt;" border="0" cellpadding="{/form/htmlAttributes/cellpadding}" cellspacing="2">
            <tr align="center"> <!-- Form header -->
                <td style="font-family:{htmlAttributes/formFont}; color: {htmlAttributes/formColor}; font-size:{/form/htmlAttributes/formFontSize}pt">
                    <input type="hidden" id="dsName" value="{dataStructureName}"/>
          ...
          <tr align="left"><!-- BTRIS Get All Button -->
             <td>
             	...
             	<!-- <img src="{$imageroot}/spacer.gif" width="1" height="10" alt="" border="0"/> -->
             </td>
          </tr>
	      ...
	      <xsl:choose>
		    ...
		    <xsl:otherwise>   
	           <xsl:choose>
			        ...
			        <xsl:otherwise>
			             <div id="{$secId}" style='display:none' parent="{$parentValue}" formrow="{$formRow}" maxdisplay="{$maximumValue}" initdisplay="{$maximumValue}">
			             <xsl:apply-templates select="section"/>
    ...
    <xsl:template match="section">
        ...
        <xsl:variable name="maxNoofColumnForEachRowInSection" select="@maxNoofColumnForEachRowInSection"/>
           ...
           <div style="display:block;position:relative;" id="sectionContainer_{$sectionid}" hideid="sectionContainer_{$sectionid}" class="ctdbSectionContainer {$tableHeaderClassName}">
    ...
    <xsl:template match="question">
		...
		<xsl:variable name="deName" select="deName"/>
	
    	
    	<td  colspan="{$floorColSpanTD}" style="width:{$widthTD}%;" class="questionContainerTD {$showTextClassName} " hideid="questionContainer_{$questionSectionNode}_{$qid}" >
	    	<table class="sectionquestionscellQuestionContainerTable"  >
	        <tr class="questionTR" dataelementname="{$deName}">
	            <xsl:choose>
	                ...
	                <xsl:otherwise>
	                    <td class="questionTextContainerTd" align="{formQuestionAttributes/htmlAttributes/align}" valign="{formQuestionAttributes/htmlAttributes/valign}" width="50%" style="font-family: {formQuestionAttributes/htmlAttributes/fontFace}; color: {formQuestionAttributes/htmlAttributes/color};">
	   					...
	   					<td class="questionInputTD" align="{formQuestionAttributes/htmlAttributes/align}" valign="{formQuestionAttributes/htmlAttributes/valign}" width="50%" style="font-family: {formQuestionAttributes/htmlAttributes/fontFace}; color: {formQuestionAttributes/htmlAttributes/color}; font-size:{/form/htmlAttributes/formFontSize}pt">
		   					<xsl:choose>
	                           <xsl:when test="$horizontalDisplay = 'true'">
	                                <br class="horizontalDisplay" style="display:none" />
	                           </xsl:when>
	                            ...
	                            <xsl:variable name="answertype" select="@answertype"/>
	                                    		...
	                                    		<xsl:when test="@answertype = 'datetime'">
	                                    		  ...
	                                    		  <xsl:otherwise>
	                                    		  	<xsl:choose>
	                                        			...
	                                        			<xsl:otherwise>	
	                                        				<input type="text" deName="{$deName}" id="S_{$questionSectionNode}_Q_{$qid}" name="S_{$questionSectionNode}_Q_{$qid}" value="{$default}" class="catfood" size="{$textboxSize}" onfocus="checkButtons(this);"/>
	                                        ...
	                                        <table>
	                                           ...
	                                           <!--  <tr><td>
	                                                <span style="float:right" class="smallText">
	                                                    ...
	                                                    <img src="{$imageroot}/spacer.gif" height="18" width="3" border="0"/>
	                                ...
	                                <xsl:when test="@type = 'Select'">
	                                        ...
	                                        <select id="S_{$questionSectionNode}_Q_{$qid}" name="S_{$questionSectionNode}_Q_{$qid}"  onfocus="checkButtons(this);" onchange="displayOther(this);">
	                                            <xsl:choose>
	                                                <xsl:when test="string($default)">
	                                                    ...
	                                                    <xsl:if test="not(contains(scoreStr, $default))">
	                                                    	<option value="{$default}" selected="selected" hideid="questionContainer_{$questionSectionNode}_{$qid}_{pvid}">
	                                                    		<xsl:value-of select="$default"/>
	                                                    	</option> 
	                                                    </xsl:if>	                                                                                                      
	                                                    <xsl:for-each select="answers/answer">
	                                    ...
	                                    <xsl:when test="@type = 'Calculated'">
	                                        ...
	                                        <input type="text" deName="{$deName}" id="S_{$questionSectionNode}_Q_{$qid}" readonly="yes" name="S_{$questionSectionNode}_Q_{$qid}" size="{$textboxSize}"  onfocus="calculate(this, '{$calc}', '{$answertype}','{$decimalPrecision}','{$conditionalForCalc}','{$isCount}');" onkeyup="userEntry(this)"/>
	                                      ...
	                                      <xsl:choose>
	                                        ...
	                                        <xsl:otherwise>
	                                   			<xsl:choose>
	                                    			...
	                                    			<xsl:otherwise>
	                                    				<input type="text" deName="{$deName}" id="S_{$questionSectionNode}_Q_{$qid}" size="{$textboxSize}" name="S_{$questionSectionNode}_Q_{$qid}" value="{$default}" onchange="applyskiprule(this, [{$questionsToSkip}], '{$skipOperator}','{$skipRule}','{$skipRuleEquals}')" onfocus="checkButtons(this);"/>
	                                    			</xsl:otherwise>
		   									 	</xsl:choose>
	                                   		</xsl:otherwise>
	                                   	</xsl:choose>
	            ...
	            <td colspan="2" id="S_{$questionSectionNode}_Q_{$qid}_link">
	                <xsl:for-each select="images/filename">
	                   <!--  <img src="{$imageroot}/questionimages/{.}" alt="Question Image" border="0"/>&#160;&#160; -->
	                    <!-- <img class="imgThumb" src="{$imageroot}/questionimages/{.}" alt="Question Image" border="0" onclick="goImgWin('{$imageroot}/questionimages/{.}',240,302,100,50)" style="cursor: pointer"/>&#160;&#160; -->
	                	<!-- <img class="imgThumb" src="{$qid}/{.}" alt="Question Image" border="0" onclick="goImgWin('{$imageroot}/response/dataCollection.action?action=getQuestionGraphic&qId={$qid}&filename={.}',240,302,100,50)" style="cursor: pointer"/>&#160;&#160;-->
	                	<!-- <img class="imgThumb" src="{$qid}/{.}" alt="Question Image" border="0"  style="cursor: pointer"/>&#160;&#160;  -->
