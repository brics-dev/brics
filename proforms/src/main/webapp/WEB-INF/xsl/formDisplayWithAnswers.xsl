...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
   ...
   <xsl:template match="formcell">
    ...
    <xsl:template match="section">
        ...
        <div style="display:block;position:relative;" id="sectionContainer_{$sectionid}" hideid="sectionContainer_{$sectionid}" class="ctdbSectionContainer ">
	        <table class="sectionContainerTable {$tableHeaderClassName}" width="100%" style="height: 100%;" border="0" cellpadding="{/form/htmlAttributes/cellpadding}" cellspacing="2">
    ...
    <xsl:template match="question">
    	...
    	<td colspan="{$floorColSpanTD}" style="width:{$widthTD}%;" class="questionContainerTD " hideid="questionContainer_{questionSectionNode}_{$qid}">
            ...
            <xsl:choose>
	                                ...
	                                <td colspan="3" align="{formQuestionAttributes/htmlAttributes/align}" valign="{formQuestionAttributes/htmlAttributes/valign}" 
	                                    ...
	                                    <xsl:choose>
	                                        <xsl:when test="@displayGrid = 'true'">
	                                            <!-- <img src="{$imageroot}/spacer.gif" border="0" height="{$imageHeight}" width="{$imageWidth}"
	                                            ...
	                                            <div style="float:left">
		                                            ...
		                                            <!-- image map -->                         
		                                            <img src="{$imageroot}/questionimages/{$imageMapFileName}" border="0" height="{$imageHeight}" width="{$imageWidth}"
		                                                style="z-index:100; position:absolute" id="imageMapImage_S_{$questionSectionNode}_Q_{$qid}"/>
		                                             <!-- image map grid -->
		                                            <img src="{$imageroot}/{$gridFileName}" border="0" height="{$imageHeight}" width="{$imageWidth}"
	                                        ...
	                                        <xsl:otherwise>
		                                        <div style="float:left">
		                                        	<!-- make image at right position --><!--  here is different with formDisplay.xsl, the hight -->
			                                        <img src="{$imageroot}/spacer.gif" border="0" height="1" width="1"
			                                             id="imageMapSpacer_S_{$questionSectionNode}_Q_{$qid}" style="z-index:-100"/>
			                                        <!-- image map -->
	                                            	<img src="{$imageroot}/questionimages/{$imageMapFileName}" border="0" height="{$imageHeight}" width="{$imageWidth}"
	                                                	 useMap="#theMap_S_{$questionSectionNode}_Q_{$qid}"/>
                ...
                <xsl:when test="@type = 'File'">
	                ...
	                <td id="replaceFileField">
	                	Replace File:
	                	<input type="file" id="S_{$questionSectionNode}_Q_{$qid}" name="fileUpload" class="fileInput"/>
                                        	...
                                        	<xsl:otherwise>
                                        		<xsl:choose>
                                        			<xsl:when test="@answertype = 'date'">
                                        		 	...
                                        		 	<xsl:otherwise>
                                        		 		<input type="text" deName="{$deName}" id="S_{$questionSectionNode}_Q_{$qid}" name="S_{$questionSectionNode}_Q_{$qid}"  size="{$textboxSize}" onfocus="checkButtons(this);" value="{$default}"/>                                      		                                    		 		
                                        		 	</xsl:otherwise>
                                        		 </xsl:choose>
                                        	</xsl:otherwise>
                                        </xsl:choose>
                                    		...
                                    		<xsl:choose>
                                        		...
                                        		<xsl:otherwise>
                                        			<xsl:choose>
                                        				...
                                        				<xsl:otherwise>
                                        					<input type="text" deName="{$deName}" id="S_{$questionSectionNode}_Q_{$qid}" name="S_{$questionSectionNode}_Q_{$qid}" value="{$default}" size="{$textboxSize}"  onfocus="checkButtons(this);" onchange="applyskiprule(this, [{$questionsToSkip}], '{$skipOperator}','{$skipRule}','{$skipRuleEquals}')"/>                                       					
                                        				</xsl:otherwise>
                                        		 	</xsl:choose>		 	
                                        		</xsl:otherwise>
                                        	</xsl:choose>
