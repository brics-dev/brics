...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        ...
        <script language="JavaScript" type="text/javascript" src="{$webroot}/common/NiftyCube/niftycube.js"></script>
        ...
        <table style="border-spacing:2px 0px;width:100%">
          ...
          <tr>
	          ...
	          <div id="controlContainer">
	           	<div id="controlContent" class="control">
	           	</div>
	           </div>
   ...
   <!--              <xsl:when test="@type = 'Patient Calendar'">
                ...
                <xsl:when test="@type = 'File'">
                    ...
                    <td align="{formQuestionAttributes/htmlAttributes/align}" valign="{formQuestionAttributes/htmlAttributes/valign}" width="50%" style="font-family: {formQuestionAttributes/htmlAttributes/fontFace}; color: {formQuestionAttributes/htmlAttributes/color}; font-size:{/form/htmlAttributes/formFontSize}pt">
	                	<input type="file" id="S_{$questionSectionNode}_Q_{$qid}" name="value(S_{$questionSectionNode}_Q_{$qid})" class="fileInput"/>
                            ...
                            <xsl:when test="$hasSkipRule = 'false'">
                                    ...
                                    <xsl:when test="@type = 'Textbox'">
                                    		<xsl:choose>
                                    			<xsl:when test="@answertype = 'datetime'">
                                        			<input type="text" id="S_{$questionSectionNode}_Q_{$qid}" name="S_{$questionSectionNode}_Q_{$qid}" value="{$default}" class="dateTimeField" size="{$textboxSize}" onfocus="checkButtons(this);"/>
                                        		</xsl:when>
                                        		<xsl:otherwise>
                                        			<xsl:choose>
                                        				<xsl:when test="@answertype = 'date'">
                                        					<input type="text" id="S_{$questionSectionNode}_Q_{$qid}" name="S_{$questionSectionNode}_Q_{$qid}" value="{$default}" class="dateField" size="{$textboxSize}" onfocus="checkButtons(this);"/>
                                        				</xsl:when>
                                        				<xsl:otherwise>
                                ...
                                <xsl:when test="@type = 'Select'">                               
                                    	...
                                    	<font size="{formQuestionAttributes/htmlAttributes/fontSize}">   <!-- add font size by Ching Heng -->
	                                        <xsl:for-each select="answers/answer">
	                                            <xsl:choose>
	                                                <xsl:when test="$default = display">
	                                                    <input type="radio" id="S_{$questionSectionNode}_Q_{$qid}" name="S_{$questionSectionNode}_Q_{$qid}" value="{display}" checked="true" title="{display/@displayToolTip}" onfocus="checkButtons(this);" onMouseDown="toggleRadio(this);" onchange="displayRadioOther(this)">
	                                                        &#xa0;<xsl:value-of select="display"/><xsl:value-of select="position"/>
	                                                    </input>
	                                                    <xsl:choose>
			                                                <xsl:when test="includeOther = 'true'"> <!-- include Other option -->
			                                                	&#xa0;<input type="text" id="S_{$questionSectionNode}_Q_{$qid}_otherBox"  name="S_{$questionSectionNode}_Q_{$qid}_otherBox" disabled="true"/>
                                    ...
                                    <xsl:when test="@type = 'Calculated'">
                                        ...
                                        <input type="text" id="S_{$questionSectionNode}_Q_{$qid}" size="{$textboxSize}"  readonly="yes" name="S_{$questionSectionNode}_Q_{$qid}" onfocus="calculate(this, '{$calc}', '{$answertype}','{$decimalPrecision}')"/>
                                    </xsl:when>
                                    <xsl:otherwise>
                            ...
                            <xsl:otherwise>
                                ...
                                <xsl:choose>
                                    ...
                                    <xsl:when test="@type = 'Textbox'">
                                    	<xsl:choose>
                                    		<xsl:when test="@answertype = 'datetime'">
                                       			 <input class="dateTimeField" type="text" id="S_{$questionSectionNode}_Q_{$qid}" size="{/formQuestionAttributes/textboxSize}" name="S_{$questionSectionNode}_Q_{$qid}" value="{$default}" onchange="applyskiprule(this, [{$questionsToSkip}], '{$skipOperator}','{$skipRule}','{$skipRuleEquals}')" onfocus="checkButtons(this);"/>
                                        	</xsl:when>
                                        	<xsl:otherwise>
                                        		<xsl:choose>
                                        			<xsl:when test="@answertype = 'date'">
                                        				<input class="dateField" type="text" id="S_{$questionSectionNode}_Q_{$qid}" size="{/formQuestionAttributes/textboxSize}" name="S_{$questionSectionNode}_Q_{$qid}" value="{$default}" onchange="applyskiprule(this, [{$questionsToSkip}], '{$skipOperator}','{$skipRule}','{$skipRuleEquals}')" onfocus="checkButtons(this);"/>
                                        			</xsl:when>
                                        			<xsl:otherwise>
                                        				<input type="text" id="S_{$questionSectionNode}_Q_{$qid}" size="{/formQuestionAttributes/textboxSize}" name="S_{$questionSectionNode}_Q_{$qid}" value="{$default}" onchange="applyskiprule(this, [{$questionsToSkip}], '{$skipOperator}','{$skipRule}','{$skipRuleEquals}')" onfocus="checkButtons(this);"/>
                                        			</xsl:otherwise>
                                        		</xsl:choose>
                                        	</xsl:otherwise>
                                        </xsl:choose>
                                    ...
                                    <xsl:when test="@type = 'Textarea'">
                                       <!--  <table style="border-spacing:2px 0px">
                                            <tr><td nowrap="true">
                                                <span style="float:right" class="smallText"><a href="Javascript:doNothing();" onClick="toggleOverflow ('S_{$questionSectionNode}_Q_{$qid}');">
                                    ...
                                    <xsl:when test="@type = 'Multi-Select'">
                                        <select id="S_{$questionSectionNode}_Q_{$qid}" name="S_{$questionSectionNode}_Q_{$qid}" multiple="true" size="5"  onfocus="checkButtons(this);" onchange="applyskiprule(this, [{$questionsToSkip}], '{$skipOperator}','{$skipRule}','{$skipRuleEquals}');displayOther(this)">
                                            <xsl:for-each select="answers/answer">
                                                <xsl:choose>
                                                    <xsl:when test="$default = display">
                                                        <option value="{display}" selected="selected" title="{display/@displayToolTip}">
                                    ...
                                    <xsl:when test="@type = 'Calculated'">
                                        ...
                                        <xsl:variable name="answertype" select="@answertype"/>
                                        <xsl:variable name="decimalPrecision" select="@decimalPrecision"/>
                                        <input type="text" id="S_{$questionSectionNode}_Q_{$qid}" size="{$textboxSize}"  name="S_{$questionSectionNode}_Q_{$qid}" readonly="yes" onfocus="calculate(this,'{$calc}', '{@answertype}','{$decimalPrecision}')" onchange="applyskiprule(this, [{$questionsToSkip}], '{$skipOperator}','{$skipRule}','{$skipRuleEquals}')"/>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        Unknown Question Type: System Error
                                    </xsl:otherwise>
