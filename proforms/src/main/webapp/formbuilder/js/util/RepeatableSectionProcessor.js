...
var RepeatableSectionProcessor = {
	...
	makeClonedSection : function(sectionModel, i) {
		...
		var id = (FormBuilder.form.sections.length + 1) * -1;
		var childSectionId = "S_" + id;
	...
	editSectionRepeatMax : function(sectionModel, repeatChildren, previous) {
		if(sectionModel.get("maxRepeatedSecs") > previous) {
			this.maxRepeatIncrease(sectionModel,sectionModel.get("maxRepeatedSecs")-previous);			
		}
		else {
			this.maxRepeatDecrease(sectionModel, repeatChildren, previous-sectionModel.get("maxRepeatedSecs"));			
		}
	},
						...
						for (var j = 0; j < questionsToSkip.length; j++) {
							if (questionsToSkip[j].indexOf(secId) != -1) {
								for (var m = 0; m < repeatedQuestions.length; m++) {
									...
									var repQuestionsToSkip = repQuestion.get("questionsToSkip");
