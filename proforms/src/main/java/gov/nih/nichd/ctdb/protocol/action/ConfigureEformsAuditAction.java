...
public class ConfigureEformsAuditAction extends BaseAction {
	...
	private static final long serialVersionUID = 1569388158929175753L;
	
	private int eformId;

	/**
	 * execute method this will serve up the basic jsp after which datatable makes ajax call to get data for audi
	 */
	...
	public String getConfigureEformAuditList() throws Exception {
		try {
			ArrayList<ConfigureEformAuditDetail> auditDetailList =
					(ArrayList<ConfigureEformAuditDetail>) getAuditList();
	...
	/**
	 * Gets the audit list. Gets the brics eform to get section and queston text
	 * 
	 * @return
	 */
						...
						if (questionId != -1) {
							...
							for (Question q : questionList) {
								...
								if (questionId == eformQuestionId) {
									...
									questionText = removeSpecialChars(questionText);
									...
									if (pvId != -1) {
										...
										for (Answer a : answerList) {
											...
											if (pvId == answerId) {
												String pvText = a.getDisplay();
												pvText = removeSpecialChars(pvText);
												auditDetail.setPvText(pvText);
												break;
											}
										}
									}
