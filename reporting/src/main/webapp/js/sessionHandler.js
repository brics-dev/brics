/**
 * This file will pop up a confirmation dialog to notify the end user 
...
var notifyThreshold = timeoutThreshold - 10;  // Notify user 10 minute before timeout.
...
var countdownIntervalId;	// Interval ID that references a function that will periodically check if the session is expired.
...
function extendSession() {
	...
	.fail(function() {
		...
		if ( !isSessionExpired() ) {
			...
			// The notification threshold has already passed. Notify the user now.
			else {
...
function checkNotifyUser() {
	if (!isSessionExpired()) {
		...
		// Check if the notify threshold has been reached, since the start time could have been reset.
