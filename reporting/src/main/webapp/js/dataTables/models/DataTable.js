	...
	loadSingleRow : function(index, cells) {
		...
		if (cells.length > 0) {
			...
			for (var j in cells) {
				...
				/*
				 *   I Thought this would work and help with performance
				console.log(j);
				var needle = {index:j};
				console.log(needle);
				console.log(this.columns.findWhere(needle));
				//Add cell to column's collection of cells
				if(typeof this.columns.findWhere({index: j}) !== 'undefined') {
					this.columns.findWhere({index: j}).get("cells").add(newCell);
				} */
