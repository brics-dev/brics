			...
			if(typeof currentSelect === "object") {
				if(currentSelect.style !== "api") {
					if(selectedRow.length <= 1) {
				     	$("#" + $elId + "_info").append('<span class=listSelectedRows> (' + selectedRow.length + ' row selected)</span>');
					}else {
				     	$("#" + $elId + "_info").append('<span class=listSelectedRows> (' + selectedRow.length + ' rows selected)</span>');
					}
				}
			}
		},
		...
		_startupDatatable: function() {
			...
			oTable.on('select', function(e, dt, type, indexes) {
				...
				if(masterSelectionList.length <= 1) {
					$("#" + (this.id) +"_info .listSelectedRows").html(' (' + masterSelectionList.length + ' row selected)');
				}else {
					$("#" + (this.id) +"_info .listSelectedRows").html(' (' + masterSelectionList.length + ' rows selected)');
				}
