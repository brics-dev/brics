(function($, window, document, undefined) {
	...
	$.fn.fnDtFilter = function(opts) {
		...
		function fnCreateSelectOptions(oTable, options, aName, iColumnIndex, bRegex, containerId, defaultVal, callBack) {
			...
			var r = '<select class="search_init select_filter form-control" id="' + aName + '"><option value="" name= '
							+ aName + ' class="search_init">' + aName + '</option>';
			var j = 0;
		...
		if ((properties.aoColumns !== null || properties.aoColumns !== 'undefined')
			...
			return (properties.aoColumns).forEach(function(aocolumn) {
				...
				case 'select':
					...
					fnCreateSelectOptions(oTable, aocolumn.options, aocolumn.name, aocolumn.columnIndex,
