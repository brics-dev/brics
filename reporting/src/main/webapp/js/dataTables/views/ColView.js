...
QTDT.ColView = BaseView.extend({
    ...
    render: function() {
        ...
        h5Html += (hType == "dataElement" && !this.model.get("doesRepeat")) ? '<div class="sorting"></div>' : "";
    ...
    createHM: function() {
        HamburgerMenu = this;
        var hbview = new QTDT.HamburgerView({ model: this.model });
        var hType = this.model.get("headerType");
        if (hType == "dataElement") {
            this.$el.children('h5').children('.hbmenu').append(hbview.render().el);
        } else if (hType == "repeatableGroup") {
            this.$el.children('h5').children('.rghbmenu').append(hbview.render().el);
        }
    },
    freeze: function() {
    ...
    adjustColWidth: function(newWidth,ellipse=false) {
