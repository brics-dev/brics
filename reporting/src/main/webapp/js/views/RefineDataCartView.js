...
QT.RefineDataCartView = BaseView.extend({
	...
	render : function() {
		...
		var view = this;
	    this.$( "#primaryForm,#secondaryForm,#tertiaryForm" ).droppable({
	    	accept : ".draggable",
	    	drop : function(event,ui) {
	    		...
	    		if($("#primaryForm").has('.draggable').length && $("#secondaryForm").has('.draggable').length 
	    				&& $formId!="primaryForm"&&$formId!="secondaryForm"&&$formId!="tertiaryForm" ){
	    ...
	    this.$( ".refineDataFormContainer" ).droppable({
	    	...
	    	drop : function(event,ui) {
	    		...
	    		if($formId=="primaryForm" ||$formId=="secondaryForm" ){
	    			$("#joinFormsButton").addClass("disabled");
		...
		$("#secondaryForm").droppable('disable');
		$("#tertiaryForm").droppable('disable');
	...
	onRunQuery : function(obj) {
		...
		for (var i = 0; i < selectedFormUris.length; i++) {
			...
			if (selectedFormUris.length == 1) {
				var view = this;
				// there should be nothing in the primary, secondary, tertiary
				this.$("#primaryForm,#secondaryForm,#tertiaryForm").find(".dataCartForm").each(function() {
	...
	joinForms : function() {
		...
		if (!this.$("#joinFormsButton").hasClass("disabled")) {
			...
			var formIds = [];
			var formIds = ["#primaryForm", "#secondaryForm", "#tertiaryForm"]
	...
	enableDroppables : function() {
		if (!$("#primaryForm").has('.draggable')) {
			$("#primaryForm").droppable('enable');
		}
		
		if($("#primaryForm").has('.draggable').length) {
			$("#secondaryForm").droppable('enable');
		}
		else {
			$("#secondaryForm").droppable('disable');
			$("#primaryForm").droppable('enable');		 
		...
		if($("#secondaryForm").has('.draggable').length) {
			$("#tertiaryForm").droppable('enable');
		}
		else {
			$("#tertiaryForm").droppable('disable');
		}
