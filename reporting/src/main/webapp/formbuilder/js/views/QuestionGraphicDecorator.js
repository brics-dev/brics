...
var QuestionGraphicDecorator = {
	...
	renderGraphicCommons : function(model) {
		...
		if(model.get("graphicNames") == null || model.get("graphicNames") == ""){
			this.$('#graphicFr').attr("src",baseUrl+"showQuestionImage!showQuestionImage.ajax");	
		}else{
			this.$('#graphicFr').attr("src",baseUrl+"showQuestionImage!showQuestionImage.ajax?questionId="+model.get("questionId")+"&imageNames="+this.model.get("graphicNames"));	
		}
