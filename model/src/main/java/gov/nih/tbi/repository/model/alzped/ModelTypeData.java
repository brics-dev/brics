...
@Entity
@Table(name = "MODEL_TYPE_DATA")
@Inheritance(strategy = InheritanceType.SINGLE_TABLE)
@DiscriminatorFormula(
        "(CASE WHEN study_id IS NOT NULL THEN 'S' " +
        " WHEN meta_study_id IS NOT NULL THEN 'M' END)")

public abstract class ModelTypeData  implements Serializable {
	
	private static final long serialVersionUID = -6766207945079509925L;


	@Id
	@GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "MODEL_TYPE_DATA_SEQ")
	@SequenceGenerator(name = "MODEL_TYPE_DATA_SEQ", sequenceName = "MODEL_TYPE_DATA_SEQ", allocationSize = 1)
	protected Long id;
