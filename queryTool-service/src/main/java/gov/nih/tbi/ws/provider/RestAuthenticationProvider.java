...
public class RestAuthenticationProvider {
	...
	/**
	 * Simple create web client utility method we can use to set a default connection and receive timeout across all the
	 * service providers
	 * 
	 * @param url
	 * @return
	...
	public RestAuthenticationProvider(String serverLocation, String proxyTicket, boolean wsSecure) {
		...
		if (QueryToolConstants.EMPTY_STRING.equals(proxyTicket)) {
			proxyTicket = null;
		}
	...
	public static String encodeUrlParam(String param) throws UnsupportedEncodingException {
		...
		if (param == null || QueryToolConstants.EMPTY_STRING.equals(param)) {
			return param;
		}
	...
	public static String buildParameters(String[] variables, String[] parameters, String ticketProperty)
		...
		for (int i = 0; i < variables.length; i++) {
			if (parameters[i] == null || QueryToolConstants.EMPTY_STRING.equals(parameters[i])) {
				continue;
			}
		...
		if (ticketProperty != null && !QueryToolConstants.EMPTY_STRING.equals(ticketProperty)) {
			if (parameterCount == 0) {
				builder.append("?");
			} else {
				builder.append("&");
