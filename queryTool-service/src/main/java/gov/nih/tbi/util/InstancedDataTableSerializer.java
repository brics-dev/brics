...
public class InstancedDataTableSerializer implements JsonSerializer<InstancedDataTable> {
	...
	public JsonElement serialize(InstancedDataTable table, Type typeOfSrc, JsonSerializationContext context) {
		...
		for (InstancedRecord record : table.getInstancedRecords()) {
			...
			if(renderFlag.isEmpty() && selectedRow != null && selectedRow.isDoHighlight()) {
			...
			if (selectedRow == null) {
				...
				columnIndex = addInstancedRow(row, renderFlag, selectedRow, columnIndex, headers, columnSpacingTracker,
						repeatingRows);
			}
			...
			for (int formIndex = 1; formIndex < headers.size(); formIndex++) {
				...
				if (formIndex > joinedRowList.size() || joinedRowList.get(formIndex - 1) == null) {
					...
					if(joinRenderFlag.isEmpty() && joinedRow != null && joinedRow.isDoHighlight()) {
			...
			// append empty spaces to the end of each row in case the expanded rg was not at the end of the table
	...
	/**
	 * Fix short name and short name+version mismatch in InstancedDataManager instead
	 */
	...
	/**
	 * When the InstancedRow is null, we need to add empty cells for each header column.
	 * 
	...
	/**
	 * This method adds an InstancedRow to the output JsonArray and return the ending column index after it is added.
	 * 
	 * @param row - JsonArray
	 * @param greyedFlag - Either empty string or QueryToolConstants.GREYED_FLAG.
	 * @param instancedRow - InstancedRow that the cell in in.
	 * @param columnIndex - The beginning column index before the row is added.
	 * @param headers - List of form headers.
	 * @param columnSpacingTracker - A map that tracks the column index of the repeating group cells
	 * @param repeatingRows - A map that stores the row index and the repeatable group row JsonArrays
		...
		for (Entry<DataTableColumn, CellValue> cellEntry : instancedRow.getCell().entrySet()) {
			...
			if (isColumnAdded(headers, column)) {
				if (cell instanceof NonRepeatingCellValue) {
					addNonRepeatingCell(row, renderFlag, instancedRow, cellEntry);
					columnIndex++;
	 ...
	 * @param instancedRow - InstancedRow that the cell in in.
	 * @param cellEntry - Map entry of the cell that contains the column information and the cell value.
	 */
	private void addNonRepeatingCell(JsonArray row, String renderFlag, InstancedRow instancedRow,
		...
		String deType = nrc.getDataElementType();

		if (deType.equals(QueryToolConstants.BIOSAMPLE_TYPE)) {
		...
		} else if (deType.equals(DataType.THUMBNAIL.getValue())) {
			addThumbnailCell(row, renderFlag, instancedRow, nrc.getValue());
		} else if (deType.equals(DataType.TRIPLANAR.getValue())) {
			addTriPlanarCell(row, renderFlag, instancedRow, nrc.getValue());
		} else if (deType.equals(DataType.FILE.getValue())) {
			addFileCell(row, renderFlag, instancedRow, nrc.getValue());
		} else {
			row.add(new JsonPrimitive(renderFlag + nrc.getValue(displayOption)));
		}
	}
	...
	private void addRepeatingCell(JsonArray row, String renderFlag, InstancedRow instancedRow, int columnIndex,
		...
		// If the cell is expanded and not greyed or has only one row, add the expanded repeatable group rows
		if (rcv.isExpanded() && (!(renderFlag.equals(QueryToolConstants.GREYED_FLAG)) || rcv.getRowCount() == 1)) {
			...
			for (InstancedRepeatableGroupRow rgRow : rcv.getRows()) {
				...
				if (rcv.getRowCount() == 1) {
					addRepeatbleGroupRow(row, renderFlag, rgRow, instancedRow, rcvDECount);
					...
					int lastColumnIndex =
							columnSpacingTracker.get(rgRowIndex) == null ? 0 : columnSpacingTracker.get(rgRowIndex);
					...
					addRepeatbleGroupRow(jsonRgRow, renderFlag, rgRow, instancedRow, rcvDECount);
	 ...
	 * @param renderFlag - Either empty string or QueryToolConstants.GREYED_FLAG.
	 * @param instancedRow - InstancedRow that the cell in in.
	 * @param value - display value of the cell, i.e. image file name in this case.
	 */
	...
	/**
	 * This method adds the expanded repeatable group data cell to the output JsonArray row.
	 * 
	 * @param row - JsonArray
	 * @param renderFlag - Either empty string or QueryToolConstants.GREYED_FLAG.
	...
	/**
	 * This method adds the collapsed repeatable group data cell to the output JsonArray row.
	 * 
	 * @param row - JsonArray
	 * @param renderFlag - Either empty string or QueryToolConstants.GREYED_FLAG.
	 * @param rowUri - rowUri of the InstancedRow that the cell in in.
	...
	/**
	 * This method adds one expanded repeatable group row to the output JsonArray row.
	 * 
	 * @param row - JsonArray
	 * @param renderFlag - Either empty string or QueryToolConstants.GREYED_FLAG.
	 ...
	 * @param cellEntry - Map entry of the cell that contains the column information and the cell value.
	 */
	private void addRepeatbleGroupRow(JsonArray row, String renderFlag, InstancedRepeatableGroupRow rgRow,
			InstancedRow instancedRow, int rcvDECount) {
		...
		if (rgRow.getCell() != null) {
			for (Entry<RepeatingCellColumn, CellValueCode> rgData : rgRow.getCell().entrySet()) {
				...
				String dataType = rgData.getKey().getType();

				if (DataType.THUMBNAIL.getValue().equals(dataType)) {
					addThumbnailCell(row, renderFlag, instancedRow, rgValue);

				} else if (DataType.TRIPLANAR.getValue().equals(dataType)) {
					addTriPlanarCell(row, renderFlag, instancedRow, rgValue);

				} else if (DataType.FILE.getValue().equals(dataType)) {
