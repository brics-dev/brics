...
public abstract class ResultSetToBean<T> implements Serializable
{
	...
	private static final Logger log = LogManager.getLogger(ResultSetToBean.class.getName());
     ...
     */
    public List<T> getBeans(ResultSet rs) throws InstantiationException, IllegalAccessException,
            IllegalArgumentException, InvocationTargetException, NoSuchMethodException
    {
        ...
        // map of the key to a list of the key of the nested object, so if the primary object is a form result then the
        ...
        while (rs.hasNext())
        {
            ...
            // if the result with the specified key has not been added yet, create new object and add the key to the
            // tracker
            ...
            for (BeanField currentField : fields)
            {
                ...
                if (FormResult.class.equals(currentField.getType()))
                {
                    ...
                    if (StudyResult.class.equals(currentField.getType()))
                    {
                        ArrayList<String> secondKeyList = secondaryKeyTracker.get(key);
                    ...
                    else
                    {
                        ...
                        value = ObjectConverter.convert(value, currentField.getType()); //attempt to convert the value into the type we want
                    }
                ...
                if (currentField.isCollection()) // if current field is a collection
                {
                    ...
                    if (newResult != null && value != null) // not a collection and is a new result
                    {
     ...
     * @throws NoSuchMethodException
     */
    public static Method getSetMethod(Class<?> type, String field, Class<?> methodType) throws SecurityException,
