...
QT.SelectionListView = BaseView.extend({
	...
	filterDataTextSearch : function() {
		...
		$.ajax({
			...
			success : function(data) {
				...
				view.updateFilterCount(models.length);
			},
			error : function() {
	...
	updateFilterCount : function(modelsLength) {
		this.config = Config.tabConfig[this.model.get("tabName")];
		if (this.config.name !== "DataElementsTab" && this.config.name !== "DefinedQueriesTab") {
			var filterCount = this.model.collection.length;

			if(typeof modelsLength !== 'undefined') {
				filterCount = modelsLength;
			}
			var filterCountTxt = "("+filterCount+" Results)";
			this.$(".filteringCount").text(filterCountTxt);
		}
	}
});