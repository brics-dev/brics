...
QT.Page = BaseModel.extend({
	...
	initialize : function() {
		...
		$.ajax({
	        ...
	        error: function(xhr, status, error) {
	        	$.ibisMessaging("dialog", "error", "There was a problem retrieving repositoryId");
	        },
	        async: false
	    });
		
		$.ajax({
	        url: "service/lastDeployed",
	        cache : false,
	        type: 'GET',
	        success: function(data) {
	        	if (typeof data.status !== "undefined") {
	        ...
	        error: function(xhr, status, error) {
	        	$.ibisMessaging("dialog", "error", "There was a problem retrieving lastDeployed");
	        },
	        async: false
	    });
	},
