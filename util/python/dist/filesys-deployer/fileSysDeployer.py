...
class ReplaceProcessor(object):
    ...
    def processRequest(self):
        ...
        result = CommonUtil.replaceFileVariable(self.trgPath, self.varName, self.config.replVal)
        return result
...
if __name__ == '__main__':
    ...
    for instLine in instFileLines:
        ...
        lineSplitRegex = re.match(r"^(.*?)\|(.*?)\|(.*?)$", instLineClean)

        command = lineSplitRegex.group(1)
        argOne = lineSplitRegex.group(2)
        argTwo = lineSplitRegex.group(3)
        args = [ argOne, argTwo ]
